...
from app.schemas.markets import CompanyComparisonRow, CompanyHistoryResponse, CompanyOverview
...
def list_companies(provider: MarketDataProvider = Depends(market_provider)) -> dict[str, list[str]]:
    return {"symbols": provider.list_symbols()}


@router.get("/company/{symbol}", response_model=CompanyOverview)
def get_company(symbol: str, provider: MarketDataProvider = Depends(market_provider)) -> CompanyOverview:
    try:
...
def get_company_history(symbol: str, provider: MarketDataProvider = Depends(market_provider)) -> CompanyHistoryResponse:
    try:
...
def compare_companies(
    symbols: list[str] = Query(..., description="Repeat the symbols query parameter for multi-company comparison."),
