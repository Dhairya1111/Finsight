              ...
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => setMobileSidebarOpen(false)}
                    className="rounded-xl border border-white/20 bg-white/10 p-2 text-blue-50 transition hover:bg-white/20 md:hidden"
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `group rounded-[18px] border px-4 py-3 transition ${
                        isActive
                          ? "border-red-200 bg-white/70 text-slate-900 shadow-[0_12px_28px_rgba(30,58,138,0.10)]"
                          : "border-transparent bg-white/35 text-slate-700 hover:border-red-200/70 hover:bg-white/70"
                      }`
                    }
                  >
                    <div className="flex items-center justify-between gap-3">
                      <span className="flex items-center gap-3">
                        <span className="rounded-xl bg-gradient-to-br from-blue-50 to-rose-50 p-2 text-red-700 transition group-hover:from-blue-100 group-hover:to-rose-100">
                      ...
                      <ArrowUpRight className="h-4 w-4 text-slate-400 transition group-hover:rotate-6 group-hover:text-red-600" />
                    </div>
                  </NavLink>
                ))}
              </nav>
              ...
              <div className="mt-3 grid gap-2">
                {shortcuts.map((item) => (
                  <Link
                    key={item.to}
                    to={item.to}
                    className="rounded-2xl border border-white/80 bg-white/65 px-4 py-3 text-sm font-medium text-slate-700 transition hover:-translate-y-0.5 hover:border-red-200 hover:text-red-700"
                  >
                    {item.label}
                  </Link>
                ))}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 md:px-8 lg:px-10">
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className={cn(
                    "hidden items-center overflow-hidden rounded-2xl border border-white/80 bg-gradient-to-r from-white/90 via-amber-50 to-rose-50 text-slate-700 shadow-[0_14px_30px_rgba(30,58,138,0.10)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_36px_rgba(30,58,138,0.14)] md:inline-flex",
                    headerCondensed
                      ? "gap-0 px-2.5 py-2.5"
                      : "gap-2 px-4 py-2.5",
                  )}
                >
                  <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-blue-700 to-rose-500 text-white shadow-[0_10px_20px_rgba(30,58,138,0.16)]">
                    {sidebarCollapsed ? (
